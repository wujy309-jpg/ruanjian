...
public class VideoCategoryServiceImpl implements VideoCategoryService {
    ...
    public void toggleEnable(Long id, Integer status) {
        VideoCategory existingCategory = videoCategoryMapper.selectById(id);
        if (existingCategory == null) {
            throw new RuntimeException("分类不存在");
        }

        if (Objects.equals(existingCategory.getStatus(), status)) {
            String str = status == 0 ? "禁用" : "启用";
            throw new RuntimeException("该分类已经[" + str + "]");
        }

        if (existingCategory.getParentId() == null || existingCategory.getParentId() == 0) {
            List<VideoCategory> videoCategories = videoCategoryMapper.selectList(new LambdaQueryWrapper<VideoCategory>().eq(VideoCategory::getParentId, existingCategory.getId()));
            for (VideoCategory videoCategory : videoCategories) {
                videoCategory.setStatus(status);
                videoCategoryMapper.updateById(videoCategory);
            }
        }

        existingCategory.setStatus(status);
        videoCategoryMapper.updateById(existingCategory);
    }
